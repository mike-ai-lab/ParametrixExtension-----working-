...
void CXmlExporter::WriteTextureFiles() {
  if (options_.export_materials()) {
    ...
    size_t texture_count = texture_helper.LoadAllTextures(
        model_, texture_writer_, options_.export_materials_by_layer());
...
static void WriteMaterialsTextureImage(
    SUMaterialRef material, const std::string& texture_image_file) {
...
void CXmlExporter::WriteMaterial(SUMaterialRef material) {
  ...
  XmlMaterialInfo info = GetMaterialInfo(material, file_.GetTextureDirectory());
...
void CXmlExporter::WriteEntities(SUEntitiesRef entities) {
  ...
  if (num_instances > 0) {
    ...
    for (size_t c = 0; c < num_instances; c++) {
      ...
      SUDrawingElementGetLayer(SUComponentInstanceToDrawingElement(instance), &layer);
      if (!SUIsInvalid(layer))
      ...
      SU_CALL(SUComponentInstanceGetTransform(instance, &instance_info.transform_));
  ...
  if (options_.export_edges()) {
    ...
    if (num_edges > 0) {
      ...
      SU_CALL(SUEntitiesGetEdges(entities, standAloneOnly, num_edges, &edges[0], &num_edges));
  ...
  if (options_.export_edges()) {
    ...
    if (num_curves > 0) {
      ...
      SU_CALL(SUEntitiesGetCurves(entities, num_curves, &curves[0], &num_curves));
...
void CXmlExporter::WriteFace(SUFaceRef face) {
  ...
  SUFaceGetUVHelper(
      face, info.has_front_texture_, info.has_back_texture_, texture_writer_, &uv_helper);
  ...
  if (num_loops == 1) {
    ...
    if (num_vertices > 0) {
      ...
      for (size_t i = 0; i < num_vertices; i++) {
        ...
        if (info.has_front_texture_) {
          SUUVQ uvq;
          if (SUUVHelperGetFrontUVQ(uv_helper, &su_point, &uvq) == SU_ERROR_NONE) {
        ...
        if (info.has_back_texture_) {
          SUUVQ uvq;
          if (SUUVHelperGetBackUVQ(uv_helper, &su_point, &uvq) == SU_ERROR_NONE) {
    ...
    for (size_t i_triangle = 0; i_triangle < num_triangles; i_triangle++) {
      ...
      for (size_t i = 0; i < 3; i++) {
        ...
        vertex_info.vertex_.SetLocation(vertices[index].x, vertices[index].y, vertices[index].z);
