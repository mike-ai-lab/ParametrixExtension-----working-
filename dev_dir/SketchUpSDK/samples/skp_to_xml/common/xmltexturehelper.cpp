...
size_t CXmlTextureHelper::LoadAllTextures(
    SUModelRef model, SUTextureWriterRef texture_writer, bool textures_from_layers) {
    ...
    if ((result == SU_ERROR_NONE) && (num_layers > 0)) {
      ...
      if (result == SU_ERROR_NONE) {
        for (size_t i = 0; i < num_layers; i++) {
          ...
          long texture_id = 0;
          result = SUTextureWriterLoadEntity(texture_writer, SULayerToEntity(layer), &texture_id);
        }
      }
    }
  } else {
    ...
    if (SU_ERROR_NONE == SUModelGetEntities(model, &model_entities)) {
      ...
      if (num_definitions > 0) {
        ...
        SUModelGetComponentDefinitions(
            model, num_definitions, &component_definitions[0], &num_definitions);

        for (size_t i = 0; i < num_definitions; i++) {
          SUComponentDefinitionRef component_definition = component_definitions[i];
...
void CXmlTextureHelper::LoadComponent(
    SUTextureWriterRef texture_writer, SUComponentDefinitionRef component) {
  SUEntitiesRef entities = SU_INVALID;
  SUComponentDefinitionGetEntities(component, &entities);
  LoadEntities(texture_writer, entities);
}

void CXmlTextureHelper::LoadEntities(SUTextureWriterRef texture_writer, SUEntitiesRef entities) {
...
void CXmlTextureHelper::LoadFaces(SUTextureWriterRef texture_writer, SUEntitiesRef entities) {
  SUResult hr;
  if (!SUIsInvalid(entities)) {
    ...
    if (num_faces > 0) {
      ...
      for (size_t i = 0; i < num_faces; i++) {
        ...
        hr = SUTextureWriterLoadFace(texture_writer, face, &front_texture_id, &back_texture_id);
      }
    }
  }
}
    ...
    if (num_instances > 0) {
      ...
      for (size_t i = 0; i < num_instances; i++) {
        ...
        if (!SUIsInvalid(instance)) {
          long texture_id = 0;
          hr = SUTextureWriterLoadEntity(
              texture_writer, SUComponentInstanceToEntity(instance), &texture_id);
        }
      }
    }
  }
...
void CXmlTextureHelper::LoadGroups(SUTextureWriterRef texture_writer, SUEntitiesRef entities) {
  if (!SUIsInvalid(entities)) {
    size_t num_groups;
...
void CXmlTextureHelper::LoadImages(SUTextureWriterRef texture_writer, SUEntitiesRef entities) {
  if (!SUIsInvalid(entities)) {
    size_t num_images = 0;
      ...
      if (SU_ERROR_NONE == SUEntitiesGetImages(entities, num_images, &images[0], &num_images)) {
        for (size_t i = 0; i < num_images; i++) {
          ...
          if (!SUIsInvalid(image)) {
            long texture_id = 0;
            SUTextureWriterLoadEntity(texture_writer, SUImageToEntity(image), &texture_id);
          }
        }
      }
    }
