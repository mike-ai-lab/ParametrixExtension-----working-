...
bool CXmlImporter::Convert(
    const std::string& xml_in, const std::string& skp_out,
...
void CXmlImporter::CreateLayers(const std::vector<XmlLayerInfo>& layer_infos) {
  for (std::vector<XmlLayerInfo>::const_iterator it = layer_infos.begin(), ite = layer_infos.end();
       it != ite; ++it) {
...
void CXmlImporter::CreateMaterials(const std::vector<XmlMaterialInfo>& mat_infos) {
  std::string texture_dir = file_.GetTextureDirectory();

  for (std::vector<XmlMaterialInfo>::const_iterator it = mat_infos.begin(), ite = mat_infos.end();
       it != ite; ++it) {
    ...
    if (info.has_texture_) {
      ...
      if (SUTextureCreateFromFile(
              &texture, texture_file.c_str(), info.texture_sscale_, info.texture_tscale_) ==
          SU_ERROR_NONE) {
...
SULayerRef CXmlImporter::FindLayer(const std::string& layer_name) const {
  std::map<std::string, SULayerRef>::const_iterator itl = layers_map_.find(layer_name);
  SULayerRef layer = SU_INVALID;
...
SUMaterialRef CXmlImporter::FindMaterial(const std::string& mat_name) const {
  std::map<std::string, SUMaterialRef>::const_iterator itm = materials_map_.find(mat_name);
  SUMaterialRef material = SU_INVALID;
...
void CXmlImporter::BuildFaceInput(
    SUGeometryInputRef geom_input, const XmlFaceInfo& face_info, size_t num_face_vertices,
    size_t& face_vertex_count, size_t& global_vertex_count) {
  ...
  if (!face_info.front_mat_name_.empty()) {
    ...
    if (face_info.has_front_texture_) {
      ...
      for (size_t i = 0; i < mat_input.num_uv_coords; ++i) {
        ...
        mat_input.uv_coords[i] =
            ConvertTextureCoords(face_info.vertices_[face_vertex_count + i].front_texture_coord_);
      }
    }
    SU_CALL(SUGeometryInputFaceSetFrontMaterial(geom_input, face_index, &mat_input));
  }
  ...
  if (!face_info.back_mat_name_.empty()) {
    ...
    if (face_info.has_back_texture_) {
      ...
      for (size_t i = 0; i < mat_input.num_uv_coords; ++i) {
        mat_input.vertex_indices[i] = global_vertex_count + i;
        mat_input.uv_coords[i] =
            ConvertTextureCoords(face_info.vertices_[face_vertex_count + i].back_texture_coord_);
      }
    }
    SU_CALL(SUGeometryInputFaceSetBackMaterial(geom_input, face_index, &mat_input));
  }
  face_vertex_count += num_face_vertices;
  global_vertex_count += num_face_vertices;
};

void CXmlImporter::CreateDefinitions(const std::vector<XmlComponentDefinitionInfo>& def_infos) {
  for (std::vector<XmlComponentDefinitionInfo>::const_iterator it = def_infos.begin(),
                                                               ite = def_infos.end();
       it != ite; ++it) {
    ...
    comp_def_map_.insert(std::make_pair(it->name_, std::make_pair(definition, &(*it))));
  }
}

bool CXmlImporter::CreateEntities(const XmlEntitiesInfo& info, SUEntitiesRef entities) {
  bool ok = true;

  // Component instances
  for (std::vector<XmlComponentInstanceInfo>::const_iterator it = info.component_instances_.begin(),
                                                             ite = info.component_instances_.end();
       it != ite; ++it) {
    const XmlComponentInstanceInfo& instance_info = *it;
    // Find the definition
    CompDefMap::const_iterator itdef = comp_def_map_.find(instance_info.definition_name_);
    ...
    if (!instance_info.layer_name_.empty()) {
      ...
      SU_CALL(SUDrawingElementSetLayer(SUComponentInstanceToDrawingElement(instance), layer));
    }
    if (!instance_info.material_name_.empty()) {
      SUMaterialRef mat = FindMaterial(instance_info.material_name_);
      SU_CALL(SUDrawingElementSetMaterial(SUComponentInstanceToDrawingElement(instance), mat));
    }
    // Set the transformation
    SU_CALL(SUComponentInstanceSetTransform(instance, &instance_info.transform_));
  ...
  for (std::vector<XmlFaceInfo>::const_iterator it = info.faces_.begin(), ite = info.faces_.end();
       it != ite; ++it) {
    const XmlFaceInfo& face_info = *it;
    // Add vertices
    for (std::vector<XmlFaceVertex>::const_iterator it = face_info.vertices_.begin(),
                                                    ite = face_info.vertices_.end();
         it != ite; ++it) {
    ...
    if (face_info.has_single_loop_) {
      ...
      BuildFaceInput(geom_input, face_info, num_vertices, face_vertex_count, global_vertex_count);
    } else {
      ...
      for (size_t tri_index = 0; tri_index < num_triangles; ++tri_index) {
        BuildFaceInput(geom_input, face_info, 3, face_vertex_count, global_vertex_count);
      }
    }
  }
  ...
  if (!info.edges_.empty()) {
    for (std::vector<XmlEdgeInfo>::const_iterator it = info.edges_.begin(), ite = info.edges_.end();
         it != ite; ++it) {
