...
bool CXmlFile::Open(const std::string& filename, bool create_new_file) {
  ...
  xml_doc_ = new SU_tinyxml2_1_0_11::XMLDocument;
  parent_node_ = xml_doc_;

  bool ok = true;

  if (!create_new_file) {
    ok = xml_doc_->LoadFile(filename.c_str()) == SU_tinyxml2_1_0_11::XML_NO_ERROR &&
...
bool CXmlFile::ReadHeader() {
  const SU_tinyxml2_1_0_11::XMLNode* node = xml_doc_->FirstChild();
  const SU_tinyxml2_1_0_11::XMLElement* elem = node->ToElement();
  bool ok = false;
  if (elem->Value() == kSkpToXMLTag) {
    int version = 0;
    ok = (elem->QueryIntAttribute(kXMLVersionTag.c_str(), &version) ==
          SU_tinyxml2_1_0_11::XML_NO_ERROR) &&
         (version == 3);
  }
  return ok;
}
...
void CXmlFile::WriteHeader(int major_ver, int minor_ver, int build_no) {
  SU_tinyxml2_1_0_11::XMLElement* elem = xml_doc_->NewElement(kSkpToXMLTag.c_str());
  SU_tinyxml2_1_0_11::XMLNode* node = xml_doc_->InsertFirstChild(elem);
...
SU_tinyxml2_1_0_11::XMLElement* CXmlFile::WriteStartTag(const char* tag) {
  SU_tinyxml2_1_0_11::XMLElement* elem = xml_doc_->NewElement(tag);
...
void CXmlFile::StartComponentDefinition(const std::string& name) {
  SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kCompDefTag.c_str());
  elem->SetAttribute(kNameTag.c_str(), name.c_str());
}

bool CXmlFile::ReadComponentDefinitionInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, bool readEntities,
...
bool CXmlFile::ReadLayerInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlLayerInfo& info) const {
  const SU_tinyxml2_1_0_11::XMLElement* elem = parent_node->ToElement();
  ...
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
...
void CXmlFile::WriteLayerInfo(const XmlLayerInfo& info) {
  SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kLayerTag.c_str());
...
bool CXmlFile::ReadColor(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, const SUColor& color) const {
...
bool CXmlFile::ReadMaterialInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlMaterialInfo& info) const {
  const SU_tinyxml2_1_0_11::XMLElement* elem = parent_node->ToElement();
  ...
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
  if (child != NULL) {
    const SU_tinyxml2_1_0_11::XMLElement* child_elem = child->ToElement();
...
void CXmlFile::WriteMaterialInfo(const XmlMaterialInfo& info) {
  ...
  SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kMaterialTag.c_str());
  elem->SetAttribute(kNameTag.c_str(), info.name_.c_str());
  ...
  if (info.has_texture_) {
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kTextureTag.c_str());
...
static bool ReadPoint(const SU_tinyxml2_1_0_11::XMLNode* parent_node, CPoint3d& point) {
  const SU_tinyxml2_1_0_11::XMLElement* elem = parent_node->ToElement();
  double x, y, z;
  if (elem->QueryDoubleAttribute(kXTag.c_str(), &x) == SU_tinyxml2_1_0_11::XML_NO_ERROR &&
      elem->QueryDoubleAttribute(kYTag.c_str(), &y) == SU_tinyxml2_1_0_11::XML_NO_ERROR &&
      elem->QueryDoubleAttribute(kZTag.c_str(), &z) == SU_tinyxml2_1_0_11::XML_NO_ERROR) {
...
bool CXmlFile::ReadEdgeInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlEdgeInfo& info) const {
  // Layer (optional)
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
  if (child == NULL)
    return false;
  if (child->Value() == kLayerTag) {
    const SU_tinyxml2_1_0_11::XMLElement* elem = child->ToElement();
...
void CXmlFile::WriteEdgeInfo(const XmlEdgeInfo& info) {
  ...
  // End points
  {
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kStartTag.c_str());
  ...
  {
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kEndTag.c_str());
...
bool CXmlFile::ReadFaceInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlFaceInfo& info) const {
  ...
  if (ok) {
    const SU_tinyxml2_1_0_11::XMLNode* vertex_node = child->FirstChild();
    while (ok && vertex_node != NULL && vertex_node->Value() == kVertexTag) {
      // Vertex position
      const SU_tinyxml2_1_0_11::XMLNode* pt_node = vertex_node->FirstChild();
      if (pt_node != NULL) {
        const SU_tinyxml2_1_0_11::XMLElement* elem = pt_node->ToElement();
        XmlFaceVertex vertex;
        if (ReadPoint(pt_node, vertex.vertex_)) {
          // Front texture coords
          const SU_tinyxml2_1_0_11::XMLNode* node = pt_node;
...
void CXmlFile::WriteFaceInfo(const XmlFaceInfo& info) {
  ...
  if (!info.front_mat_name_.empty()) {
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kFrontMaterialTag.c_str());
  ...
  if (!info.back_mat_name_.empty()) {
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kBackMaterialTag.c_str());
  ...
  if (info.has_single_loop_) {
    ...
    SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kTrianglesTag.c_str());
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    {
      SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kPointTag.c_str());
    ...
    if (info.has_front_texture_) {
      SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kFrontTextureCoordsTag.c_str());
    ...
    if (info.has_back_texture_) {
      SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kBackTextureCoordsTag.c_str());
...
bool CXmlFile::ReadCurveInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlCurveInfo& info) const {
  bool ok = true;
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
...
bool CXmlFile::ReadTransformation(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, SUTransformation& transform) const {
  const SU_tinyxml2_1_0_11::XMLElement* elem = parent_node->LastChildElement();
...
void CXmlFile::WriteTransformation(const SUTransformation& transform) {
  SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kTransformTag.c_str());
  for (int col = 0; col < 4; ++col) {
    for (int row = 0; row < 4; ++row) {
      std::string tag = MakeMatrixAttribName(row, col);
...
bool CXmlFile::GetModelInfo(XmlModelInfo& model_info) const {
  ...
  SU_tinyxml2_1_0_11::XMLNode* child = xml_doc_->FirstChild();
  while (child != NULL) {
...
bool CXmlFile::ReadLayers(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, std::vector<XmlLayerInfo>& layer_infos) const {
  bool ok = true;
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
    XmlLayerInfo info;
...
bool CXmlFile::ReadMaterials(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, std::vector<XmlMaterialInfo>& mat_infos) const {
  bool ok = true;
  const SU_tinyxml2_1_0_11::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
...
bool CXmlFile::ReadComponentDefinitions(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node,
...
void CXmlFile::WriteComponentInstanceInfo(const XmlComponentInstanceInfo& info) {
  SU_tinyxml2_1_0_11::XMLElement* elem = WriteStartTag(kComponentInstanceTag.c_str());
...
bool CXmlFile::ReadComponentInstanceInfo(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlComponentInstanceInfo& info) const {
  bool ok = true;
...
bool CXmlFile::ReadEntities(
    const SU_tinyxml2_1_0_11::XMLNode* parent_node, XmlEntitiesInfo& entities) const {
  bool ok = true;
