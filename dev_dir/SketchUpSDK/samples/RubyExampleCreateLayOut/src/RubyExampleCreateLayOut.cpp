...
VALUE create_layout_doc(VALUE self, VALUE skp_path) {
  ...
  LOStyleSetDimensionRotationAlignment(
      lo_dimension_style_ref, LODimensionRotationAlignment_Horizontal);
  LOStyleSetDimensionVerticalAlignment(lo_dimension_style_ref, LODimensionVerticalAlignment_Below);
  // Set the units to architectural inches with a 1/16" precision.
  LOStyleSetDimensionUnits(lo_dimension_style_ref, LODimensionUnits_ArchitecturalInches, 1. / 16.);
...
VALUE add_linear_dimension(
    VALUE self, VALUE start_point, VALUE end_point, VALUE start_point_pid, VALUE end_point_pid,
    VALUE text_offset) {
  ...
  LOPoint3D start_3d = { GetRubyDouble(start_arr[0]), GetRubyDouble(start_arr[1]),
                         GetRubyDouble(start_arr[2]) };

  VALUE* end_arr = RARRAY_PTR(end_point);
  LOPoint3D end_3d = { GetRubyDouble(end_arr[0]), GetRubyDouble(end_arr[1]),
                       GetRubyDouble(end_arr[2]) };

  VALUE* offset_arr = RARRAY_PTR(text_offset);
  SUVector3D offset_3d = { GetRubyDouble(offset_arr[0]), GetRubyDouble(offset_arr[1]),
  ...
  LOPoint3D start_offset_3d = { offset_3d.x + start_3d.x, offset_3d.y + start_3d.y,
  ...
  LOSketchUpModelConvertModelPointToPaperPoint(lo_skp_ref, &start_offset_3d, &start_offset_2d);

  double start_offset_length = sqrt(
      (start_2d.x - start_offset_2d.x) * (start_2d.x - start_offset_2d.x) +
      (start_2d.y - start_offset_2d.y) * (start_2d.y - start_offset_2d.y));
  ...
  LOPoint2D start_offset = { start_offset_2d.x - start_2d.x, start_offset_2d.y - start_2d.y };
  ...
  double dot_product = (start_end_perp.x * start_offset.x) + (start_end_perp.y * start_offset.y);
  if (dot_product < 0.)
    start_offset_length *= -1.;

  SUResult result;
  result = LOLinearDimensionCreate(&lo_dimension_ref, &start_2d, &end_2d, start_offset_length);
  ...
  LODocumentAddEntity(lo_document_ref, lo_entity_ref, lo_dimension_layer_ref, lo_page_ref);
  ...
  if (!start_pid_str.empty()) {
    LOConnectionPointCreateFromPID(&start_connection, lo_skp_ref, &start_3d, start_pid_str.c_str());
  } else {
  ...
  if (!end_pid_str.empty()) {
    LOConnectionPointCreateFromPID(&end_connection, lo_skp_ref, &end_3d, end_pid_str.c_str());
  } else {
    // If the pid path isn't found, an empty string is passed in. This will cause
    // the connections to be in "soft disconnect" mode, meaning that connection
    // points will not update with entity they connect.
...
VALUE add_label(VALUE self, VALUE text, VALUE anchor, VALUE target, VALUE point_pid) {
  ...
  LOPoint3D anchor_3d = { GetRubyDouble(anchor_arr[0]), GetRubyDouble(anchor_arr[1]),
                          GetRubyDouble(anchor_arr[2]) };

  VALUE* target_arr = RARRAY_PTR(target);
  LOPoint3D target_3d = { GetRubyDouble(target_arr[0]), GetRubyDouble(target_arr[1]),
                          GetRubyDouble(target_arr[2]) };

  LOPoint2D anchor_2d, target_2d;
  LOSketchUpModelConvertModelPointToPaperPoint(lo_skp_ref, &anchor_3d, &anchor_2d);
  LOSketchUpModelConvertModelPointToPaperPoint(lo_skp_ref, &target_3d, &target_2d);
  ...
  SUResult result;
  result = LOLabelCreateAtPoint(
      &lo_label_ref, &anchor_2d, anchor_type, text_string.c_str(), leader_line_type, &target_2d);
  ...
  LODocumentAddEntity(lo_document_ref, lo_entity_ref, lo_label_layer_ref, lo_page_ref);

  // Now that the label has been added to the document, we can connect it to
  // the SketchUp model.
  LOConnectionPointRef connection = SU_INVALID;
  if (!pid_str.empty()) {
    LOConnectionPointCreateFromPID(&connection, lo_skp_ref, &target_3d, pid_str.c_str());
  } else {
    // If the pid path isn't found, an empty string is passed in. This will cause
    // the connections to be in "soft disconnect" mode, meaning that connection
    // points will not update with entity they connect.
...
VALUE save_layout_doc() {
  ...
  SUResult result;
  result = LODocumentSaveToFile(lo_document_ref, lo_filepath.c_str(), LODocumentVersion_Current);
...
extern "C" void Init_RubyExampleCreateLayOut() {
  VALUE mRubyExampleCreateLayOut = rb_define_module("RubyExampleCreateLayOut");
  rb_define_module_function(
      mRubyExampleCreateLayOut, "create_layout_doc", VALUEFUNC(create_layout_doc), 1);
  rb_define_module_function(
      mRubyExampleCreateLayOut, "save_layout_doc", VALUEFUNC(save_layout_doc), 0);
  rb_define_module_function(
      mRubyExampleCreateLayOut, "add_linear_dimension", VALUEFUNC(add_linear_dimension), 5);
  rb_define_module_function(mRubyExampleCreateLayOut, "add_label", VALUEFUNC(add_label), 4);
}
