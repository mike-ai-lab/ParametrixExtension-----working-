...
int main() {
  ...
  for (size_t count = 1; count < scene_count; ++count) {
    ...
    SUStringGetUTF8(
        scene_names[count], scene_name_length + 1, scene_name_chars, &scene_name_length);
  ...
  if (path_end != std::string::npos) {
    std::string filename = full_path.substr(path_end + 1, strlen(skp_file_path) - path_end - 5);
    std::string path = full_path.substr(0, path_end);
    path.append("/");
    path.append(filename);
    path.append(".layout");
    result = LODocumentSaveToFile(document_ref, path.c_str(), LODocumentVersion_Current);
