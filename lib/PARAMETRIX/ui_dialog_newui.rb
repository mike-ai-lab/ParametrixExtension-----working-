...
module PARAMETRIX
  ...
  def self.show_html_dialog(multi_face_position)
    # Clear any existing dialog cache
    if @@current_dialog && @@current_dialog.visible?
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-3">
                    ...
                    <div class="form-field col-span-2">
                        <div class="flex items-center mb-2">
                            <input type="checkbox" id="randomize_thickness" class="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" #{@@randomize_thickness ? 'checked' : ''} />
                            <label for="randomize_thickness" class="text-sm text-gray-700">Randomize Panel Thickness (Stone Wall Effect)</label>
                        </div>
                        <div class="form-field" id="thickness_variation_field" style="#{@@randomize_thickness ? '' : 'opacity: 0.5; pointer-events: none;'}">
                            <label for="thickness_variation" class="text-xs font-medium text-gray-600 mb-1 block">Thickness Variation (Min;Base;Max)</label>
                            <input type="text" id="thickness_variation" value="#{@@thickness_variation}" class="rounded-lg" placeholder="15;20;25" />
                        </div>
                    </div>
                    <div class="form-field">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set up thickness randomization toggle
        document.getElementById('randomize_thickness').addEventListener('change', function() {
            const field = document.getElementById('thickness_variation_field');
            if (this.checked) {
    ...
    function collectSettings() {
        ...
        return {
            ...
            randomize_thickness: document.getElementById('randomize_thickness').checked,
            thickness_variation: document.getElementById('thickness_variation').value || '',
    ...
    function applyPresetToUI(presetData) {
        ...
        if (presetData.hasOwnProperty('randomize_thickness')) document.getElementById('randomize_thickness').checked = presetData.randomize_thickness;
        if (presetData.thickness_variation) document.getElementById('thickness_variation').value = presetData.thickness_variation;
    ...
    # Force cache clearing with unique preferences key
    timestamp = Time.now.to_f.to_s.gsub('.', '')
    dialog = UI::HtmlDialog.new({
      :dialog_title => "PARAMETRIX v1.2-RAILS-FIX Configuration",
      :preferences_key => "com.parametrix.newui.layout.#{timestamp}",
      :scrollable => false,
      :resizable => false,
      :width => 420,
      :height => 580, 
    ...
    dialog.add_action_callback("apply_settings") { |action_context, json_string|
      ...
      @@randomize_thickness = settings["randomize_thickness"]
      ...
      @@single_row_randomize_thickness = settings["randomize_thickness"]
      @@single_row_thickness_variation = settings["thickness_variation"]
    ...
    puts "[PARAMETRIX] Dialog loaded - Version: v1.2-RAILS-FIX, Build: RAILS-SEPARATE-#{Time.now.strftime('%Y%m%d-%H%M%S')}"
  end
