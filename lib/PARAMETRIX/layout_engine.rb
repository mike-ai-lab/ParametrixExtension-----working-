...
module PARAMETRIX
    ...
    if !is_preview
      ...
      # Recalculate stone bounds AND transform from TRIMMED faces for rail positioning
      stone_min_y_for_rails = Float::INFINITY
      stone_max_y_for_rails = -Float::INFINITY
      ...
      extrude_entities.grep(Sketchup::Face).each do |f|
        f.vertices.each do |v|
          local_pt = v.position.transform(inv_transform)
          stone_min_y_for_rails = [stone_min_y_for_rails, local_pt.y].min
          stone_max_y_for_rails = [stone_max_y_for_rails, local_pt.y].max
        end
      end
    ...
    rails_group = create_rails_for_face(face_data, main_group, face_group, rail_material, unit_conversion, face_index, cavity_distance_su, [face_data], local_bounds, face_transform, original_normal, single_row_mode, stone_min_y_for_rails, stone_max_y_for_rails, joint_width_su)
  ...
  def self.create_multi_face_unified_layout(multi_face_position, redo_mode = 0, options = {})
    ...
    begin
      ...
      # Disable trimming for multi-face corner connections
      @@global_has_openings = false
      puts "[PARAMETRIX] Multi-face mode: preserve_corners=#{@@preserve_corners}"
